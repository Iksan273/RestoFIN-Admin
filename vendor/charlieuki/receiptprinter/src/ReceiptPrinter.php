...
class ReceiptPrinter
{
    private $printer;
    private $logo;
    private $meja; // Menambah properti $meja
    private $store;
    private $items;
    private $currency = 'Rp';
    private $subtotal = 0;
    ...
    function __construct()
    {
        $this->printer = null;
        $this->items = [];
        $this->meja = ''; // Menginisialisasi $meja
    }
    ...
    public function setNoMeja($meja)
    {
        $this->meja = $meja;
    }
    ...
    public function addKitchenItem($name, $qty)
    {
        // Here, we pass null for the price since it’s not needed for the kitchen invoice
        $item = new Item($name, $qty, null);

        // Add item to the kitchen items array
        $this->items[] = $item;
    }
    ...
    public function getPrintableHeader($left_text, $is_double_width = false)
    {
        $cols_width = $is_double_width ? 8 : 16;

        return str_pad($left_text, $cols_width);
    }
    ...
    public function printReceipt($with_items = true)
    {
        if ($this->printer) {
            ...
            $footer = "Thank you and Have a Nice Day!\n";
            ...
            $this->printer->feed();
            $this->printer->text("Website");
            $this->printer->feed();
            $this->printer->text("VinResto.com");
            $this->printer->feed();
            // Print receipt date
    ...
    public function printKitchen($with_items = true)
    {
        if ($this->printer) {
            $header = $this->getPrintableHeader(
                'Order: ' . $this->transaction_id
            ...
            // Print receipt title
            $this->printer->setEmphasis(true);
            $this->printer->text("KITCHEN INVOICE\n");
            $this->printDashedLine();
            $this->printer->setEmphasis(false);
            $this->printer->feed();
            ...
            if ($with_items) {
                $this->printer->setJustification(Printer::JUSTIFY_LEFT);
                foreach ($this->items as $item) {
                    $this->printer->text($item->getQty() . 'x ' . $item->getName() . "\n");
                }
                $this->printer->feed(2);
            }
